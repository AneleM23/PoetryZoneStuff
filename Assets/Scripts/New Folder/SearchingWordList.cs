   ...
   private void Start()
    ...
    private Vector3 GetSquareScale(Vector3 defaultScale)
    {
        ...
        while (ShouldScaleDown(finalScale))
        {
            ...
            if(finalScale.x <= 0f || finalScale.y <= 0)
            {
                finalScale.x = adjustment;
                finalScale.y = adjustment;
    ...
    private bool ShouldScaleDown(Vector3 targetScale)
    {
        ...
        //Make sure all the squares fit in the rectangle 

        if(totalSquareHeight > parentRect.rect.height)
        {
            while(totalSquareHeight > squareRect.rect.height)
            {
    ...
    private void SetWordsPosition()
    {
        ...
        foreach( var word in _words )
        {
            ...
            var positionX = startPosition.x + _wordOffset.x * columnNumber;
            var positionY = startPosition.y - _wordOffset.y * rowNumber;
